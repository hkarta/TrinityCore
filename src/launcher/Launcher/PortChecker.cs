...
namespace Launcher
{
    public static class PortChecker
    {
        public static Process GetProcess(int port)
        {
            ...
            {
                if(tcpRow.LocalEndPoint.Port.ToString() == port.ToString() ||
                    tcpRow.RemoteEndPoint.Port.ToString() == port.ToString())
                        return Process.GetProcessById(tcpRow.ProcessId);
            }

            return null;
        }

        public static int GetPort(Process proc)
        {
            foreach (TcpRow tcpRow in ManagedIpHelper.GetExtendedTcpTable(true))
            {
                if (tcpRow.ProcessId == proc.Id)
