...
public:
    ...
    bool dropQuest(char *str) { return HandleQuestRemove(str); }
};
...
uint32 PlayerbotAI::getSpellId(const char* args, bool master) const
{
    ...
    for (PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        if (spell->state == PLAYERSPELL_REMOVED || spell->disabled || !spell->active) //[NOTSURE]
            continue;
...
uint32 PlayerbotAI::getPetSpellId(const char* args) const
{
    ...
    for (PetSpellMap::iterator itr = pet->m_spells.begin(); itr != pet->m_spells.end(); ++itr)
    {
		...
		if (itr->second.state == PETSPELL_REMOVED || !itr->second.active)//[NOTSURE]
            continue;
...
uint32 PlayerbotAI::initSpell(uint32 spellId)
{
	...
	next = sSpellMgr->GetNextSpellInChain(spellId);
    ...
    if (next == 0)
    {
        ...
        //Spell *spell = new Spell(m_bot, sSpellMgr->GetSpellInfo(spellId), false);
		SpellRangeEntry const* srange = sSpellRangeStore.LookupEntry(pSpellInfo->rangeIndex);
		float range = sSpellMgr->GetSpellInfo(spellId)->GetMaxRange(sSpellMgr->GetSpellInfo(spellId)->IsPositive()); // [NOTSURE]
...
void PlayerbotAI::SendQuestNeedList()
{
    ...
    {
		uint32 cid = itr->first;
        CreatureTemplate const* cInfo = sObjectMgr->GetCreatureTemplate(cid);
        if (cInfo)
        {
        ...
        {
			uint32 gid = itr->first;
            GameObjectTemplate const* gInfo = sObjectMgr->GetGameObjectTemplate(gid);
            if (gInfo)
            {
    ...
    case SMSG_FORCE_RUN_SPEED_CHANGE:
        {
            ...
            {
                //Player Part
                if (!GetMaster()->GetAuraEffectsByType(SPELL_AURA_MOUNTED).empty()) //[NOTSURE]
                {
                    int32 master_speed1 = 0;
                    int32 master_speed2 = 0;
					master_speed1 = GetMaster()->GetAuraEffectsByType(SPELL_AURA_MOUNTED).front()->GetSpellInfo()->Effects[1].BasePoints;  // GetSpellProto()->EffectBasePoints[1];
                    master_speed2 = GetMaster()->GetAuraEffectsByType(SPELL_AURA_MOUNTED).front()->GetSpellInfo()->Effects[2].BasePoints; //[STRANGE]
                    ...
                    for (PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
                    {
                        uint32 spellId = itr->first;
						PlayerSpell *spell = itr->second;
						if (spell->state == PLAYERSPELL_REMOVED || spell->disabled || !spell->active)
                            continue;
                        const SpellEntry* pSpellInfo = sSpellStore.LookupEntry(spellId);
                        if (!pSpellInfo)
                            continue;
    ...
    case SMSG_MOVE_SET_CAN_FLY:
        {
            ...
            if (guid != m_bot->GetGUID())
                return;
            m_bot->m_movementInfo.AddMovementFlag(MOVE_FLIGHT);
    ...
    case SMSG_MOVE_UNSET_CAN_FLY:
        {
            ...
            if (guid != m_bot->GetGUID())
                return;
            m_bot->m_movementInfo.RemoveMovementFlag(MOVE_FLIGHT);
    ...
    case SMSG_LOOT_RELEASE_RESPONSE:
        {
            ...
            if (guid == m_lootCurrent)
            {
				...
				if (c && c->GetCreatureTemplate()->SkinLootId && !c->lootForBody) ///[NOTSURE]
                {
...
Item* PlayerbotAI::FindMount(uint32 matchingRidingSkill) const
{
    ...
    for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
    {
        ...
        if (pItem)
        {
            ...
            else if (!partialMatch || (partialMatch && partialMatch->GetTemplate()->RequiredSkillRank < pItemProto->RequiredSkillRank))
                partialMatch = pItem;
        }
    }
...
void PlayerbotAI::GetCombatTarget(Unit* forcedTarget)
{
    ...
    if (m_botState != BOTSTATE_COMBAT)
    {
        ...
        // m_lootCurrent = uint64(); This was clearing loot target, causing bots to leave corpses unlooted if interupted by combat. Needs testing.
...
void PlayerbotAI::DoCombatMovement()
{
    ...
    float targetDist = m_bot->GetDistance(m_targetCombat);//m_bot->GetCombatDistance(m_targetCombat);
...
void PlayerbotAI::DoLoot()
{
    ...
    if ((m_lootCurrent == 0 || m_lootCurrent == NULL) && m_lootTargets.empty())
    {
    ...
    if ((m_lootCurrent == 0 || m_lootCurrent == NULL))
    {
        m_lootCurrent = m_lootTargets.front();
        m_lootTargets.pop_front();
    }

    WorldObject *wo; // [NOTSURE]= m_bot->GetMap()->GetWorldObject(m_lootCurrent);
	wo = ObjectAccessor::GetWorldObject(*wo, m_lootCurrent);

    Creature *c = m_bot->GetMap()->GetCreature(m_lootCurrent);
    GameObject *go = m_bot->GetMap()->GetGameObject(m_lootCurrent);

    // clear creature or object that is not spawned or if not creature or object
	if ((c && c->IsVisible()) || (go && !go->isSpawned()) || (!c && !go)) // was IsDespawned()
    {
        m_lootCurrent = uint64();
        return;
    }
    ...
    if (m_bot->GetDistance(wo) < INTERACTION_DISTANCE)
    {
        ...
        if (keyFailed && skillFailed && forceFailed)
        {
            //DEBUG_LOG ("[PlayerbotAI]: DoLoot attempts failed on [%s]",
                go ? go->GetGOInfo()->name : c->GetCreatureTemplate()->Name;
            m_lootCurrent = uint64();
...
void PlayerbotAI::AcceptQuest(Quest const *qInfo, Player *pGiver)
{
    ...
    if (!pGiver->CanShareQuest(qInfo->GetQuestId()))
    {
        ...
        //m_bot->ClearDividerGuid(); [HAVENOFUCKINGIDEA]
        return;
    }
...
void PlayerbotAI::CombatOrderRestore(uint8 Prim, uint8 Sec)
{
    ...
    else
    {
        ...
        uint64 PrimtargetGUID = uint64(fields[2].GetUInt64());
        uint64 SectargetGUID = uint64(fields[3].GetUInt64());
    ...
    Unit *gtarget = NULL;
    uint64 NotargetGUID = m_bot->GetGUID();
...
void PlayerbotAI::UpdateAI(const uint32 /*p_time*/)
{
    ...
    if (!m_bot->isAlive() && !m_bot->InArena())
    {
        ...
        else if (m_botState == BOTSTATE_DEAD)
        {
			...
			corpse->GetPosition(loc.m_positionX, loc.m_positionY, loc.m_positionZ);
            m_bot->TeleportTo(loc.m_mapId, loc.m_positionX, loc.m_positionY, loc.m_positionZ, m_bot->GetOrientation());
    ...
    else // bot still alive
    {
        ...
        else if (m_botState == BOTSTATE_TAME)
        {
            ...
            if (m_bot->GetPetGUID() || spell->CheckCast(true) != SPELL_CAST_OK || !pTarget ||
                pTarget->isDead() || !m_bot->IsInMap(pTarget) || !(((Creature *) pTarget)->GetCreatureTemplate()->type_flags & CREATURE_TYPEFLAGS_TAMEABLE))
            {
                MovementReset();
                m_bot->SetSelection(uint64());
                SetState(BOTSTATE_NORMAL);
            ...
            else if (!m_bot->HasAura(TAME_BEAST_1, EFFECT_1))
            {
...
bool PlayerbotAI::CastSpell(uint32 spellId)
{
        ...
        pSpellInfo->Effect[0] == SPELL_EFFECT_SKINNING)
    {
        if (!m_lootCurrent.IsEmpty())
        {
            ...
            *packet << uint32(target_type);
			packet->appendPackGUID(m_lootCurrent);
    ...
    else
    {
		...
		m_bot->CastSpell(pTarget, sSpellMgr->GetSpellInfo(pSpellInfo->Id), true);       // actually cast spell
    }
...
bool PlayerbotAI::CastPetSpell(uint32 spellId, Unit* target)
{
    ...
    pet->CastSpell(pTarget, sSpellMgr->GetSpellInfo(pSpellInfo->Id), false);
...
void PlayerbotAI::findNearbyGO()
{
    ...
    {
        uint32 entry = *(itr);
        GameObjectTemplate const * gInfo = ObjectMgr::GetGameObjectTemplate(entry);
        bool questGO = false;
        uint8 needCount = 0;
...
void PlayerbotAI::findNearbyCreature()
{
    ...
    for (std::list<Creature*>::iterator iter = creatureList.begin(); iter != creatureList.end(); iter++)
    {
        ...
        for (std::list<enum NPCFlags>::iterator itr = m_findNPC.begin(); itr != m_findNPC.end(); itr = m_findNPC.erase(itr))
        {
            ...
            WorldObject *wo = m_bot->GetMap()->GetWorldObject(currCreature->GetGUID());
            ...
            if (m_bot->GetDistance(wo) < INTERACTION_DISTANCE)
            {
                ...
                // DEBUG_LOG("%s is interacting with (%s)",m_bot->GetName(),currCreature->GetCreatureTemplate()->Name);
...
void PlayerbotAI::UseItem(Item *item, uint32 targetFlag, uint64 targetGUID)
{
    ...
    if (item->GetTemplate()->Flags & ITEM_FLAG_LOOTABLE && spellId == 0)
    {
...
void PlayerbotAI::EquipItem(Item* src_Item)
{
    ...
    // DEBUG_LOG("PlayerbotAI::EquipItem: %s in srcbag = %u, srcslot = %u", src_Item->GetTemplate()->Name1, src_bagIndex, src_slot);
...
void PlayerbotAI::CreatureLocalization(std::string& creatureName, const uint32 entry) const
{
    ...
    CreatureLocale const *pCreatureTemplate = sObjectMgr.GetCreatureLocale(entry);
    if (pCreatureTemplate)
        if (pCreatureTemplate->Name.size() > loc && !pCreatureTemplate->Name[loc].empty())
        {
            const std::string title = pCreatureTemplate->Name[loc];
            if (Utf8FitTo(title, wnamepart))
...
bool PlayerbotAI::RemoveAuction(const uint32 auctionid)
{
    ...
    if (result)
    {
        ...
        MailDraft(msgAuctionCanceledOwner.str(), "")    // TODO: fix body
            .AddItem(pItem)
            .SendMailTo(MailReceiver(uint64(HIGHGUID_PLAYER, auction->owner)), auction, MAIL_CHECK_MASK_COPIED);
...
std::string PlayerbotAI::AuctionResult(std::string subject, std::string body)
{
    ...
    ItemTemplate const *pProto = ObjectMgr::GetItemTemplate(a_info[0]);
    if (!pProto)
        return out.str();
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (expireTime > currtime)
            {
                ...
                if (bidder)
                {
                    uint64 guid = uint64(HIGHGUID_PLAYER, bidder);
...
void PlayerbotAI::Buy(uint64 vendorguid, const uint32 itemid)
{
    ...
    for (uint8 vendorslot = 0; vendorslot < numitems; ++vendorslot)
    {
        ...
        if (crItem)
        {
            ...
            ItemTemplate const *pProto = ObjectMgr::GetItemTemplate(itemid);
            if (pProto)
            {
                ...
                if (pProto->Flags & ITEM_FLAG_BOA)
                    ...
                    if (pProto->RequiredReputationFaction && uint32(m_bot->GetReputationRank(pProto->RequiredReputationFaction)) >= pProto->RequiredReputationRank)
                        ...
                        if (uint32 newItemId = sObjectMgr.GetItemConvert(itemid, m_bot->getRaceMask()))
                            pProto = ObjectMgr::GetItemTemplate(newItemId);
            ...
            if (m->HasItems())
            {
                ...
                for (MailItemInfoVec::const_iterator itr = m->items.begin(); itr != m->items.end(); )
                {
                    ...
                    if (res == EQUIP_ERR_OK)
                    {
                        ...
                        {
                            uint64 sender_guid = uint64(HIGHGUID_PLAYER, m->sender);
                            ...
                            if (GetMaster()->GetSession()->GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                            {
                                ...
                                sLog.outCommand(GetMaster()->GetSession()->GetAccountId(), "GM %s (Account: %u) receive mail item: %s (Entry: %u Count: %u) and send COD money: %u to player: %s (Account: %u)",
                                    GetMaster()->GetSession()->GetPlayerName(), GetMaster()->GetSession()->GetAccountId(), item->GetTemplate()->Name1, item->GetEntry(), item->GetCount(), m->COD, sender_name.c_str(), sender_accId);
                            }
                            else if (!sender)
                                sender_accId = sAccountMgr.GetPlayerAccountIdByGUID(sender_guid);
...
void PlayerbotAI::_HandleCommandUse(std::string &text, Player &fromPlayer)
{
    ...
    if (tool)
    {
        ...
        findItemsInEquip(itemIds, itemList);
        extractGOinfo(text, m_lootTargets);
        // DEBUG_LOG("tool (%s)",tool->GetTemplate()->Name1);
        ...
        if (!itemList.empty())
        {
            ...
            if (itarget)
            {
                // DEBUG_LOG("target (%s)",itarget->GetTemplate()->Name1);
        ...
        else if (!m_lootTargets.empty())
        {
            ...
            if (go)
                // DEBUG_LOG("tool (%s) on target gameobject (%s)",tool->GetTemplate()->Name1,go->GetGOInfo()->name);
                UseItem(tool, TARGET_FLAG_OBJECT, gotarget);  // on gameobject
        }
        else if (unit)
            // DEBUG_LOG("tool (%s) on selected target unit",tool->GetTemplate()->Name1);
            UseItem(tool, unit);  // on unit
        else
            // DEBUG_LOG("tool (%s) on self",tool->GetTemplate()->Name1);
    ...
    if (HasCollectFlag(COLLECT_FLAG_NEAROBJECT))
    {
        ...
        if (!m_collectObjects.empty())
        {
            ...
            for (BotEntryList::iterator itr = m_collectObjects.begin(); itr != m_collectObjects.end(); ++itr)
            {
                ...
                GameObjectTemplate const * gInfo = ObjectMgr::GetGameObjectTemplate(objectentry);
                strobjects += ", ";
        ...
        for (std::list<uint32>::iterator it = m_spellsToLearn.begin(); it != m_spellsToLearn.end(); ++it)
        {
            ...
            if (SkillLine->categoryId == SKILL_CATEGORY_PROFESSION && *it == SKILL_ENCHANTING)
                for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
                {
                    ...
                    if (IsPrimaryProfessionSkill(*it) && spellInfo->Effect[EFFECT_0] != SPELL_EFFECT_ENCHANT_ITEM)
                        continue;
...
void PlayerbotAI::_HandleCommandSurvey(std::string &text, Player &fromPlayer)
{
    ...
    if (result)
    {
        do
        {
            ...
            GameObject *go = m_bot->GetMap()->GetGameObject(uint64(HIGHGUID_GAMEOBJECT, entry, guid));
            if (!go)
                continue;
