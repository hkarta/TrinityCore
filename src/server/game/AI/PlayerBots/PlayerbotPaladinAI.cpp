...
bool PlayerbotPaladinAI::HealTarget(Unit *target)
{
    ...
    if (PURIFY > 0 && m_ai->GetCombatOrder() != PlayerbotAI::ORDERS_NODISPEL)
    {
		...
		for(Unit::AuraMap::const_iterator itr = auras.begin(); itr!=auras.end(); ++itr)
        {
            ...
            else if ((1 << holder->Dispel) & dispelMask3 & (DISPEL == CLEANSE))
            {
                if (holder->Dispel == DISPEL_MAGIC)
                    m_ai->CastSpell(DISPEL, *target);
                return false;
            }
        }
...
bool PlayerbotPaladinAI::DoNextCombatManeuver(Unit *pTarget)
{
    ...
    if (GetMaster()->isAlive())
        if (masterHP < 25 && HAND_OF_PROTECTION > 0 && !GetMaster()->HasAura(FORBEARANCE, EFFECT_0) && !GetMaster()->HasAura(HAND_OF_PROTECTION, EFFECT_0) && !GetMaster()->HasAura(DIVINE_PROTECTION, EFFECT_0) && !GetMaster()->HasAura(DIVINE_SHIELD, EFFECT_0))
    ...
    switch (spec)
    {
        ...
        case PALADIN_SPEC_RETRIBUTION:
            ...
            else if (ART_OF_WAR > 0 && EXORCISM > 0 && !m_bot->HasSpellCooldown(EXORCISM) && m_bot->HasAura(ART_OF_WAR, EFFECT_0))
            {
            ...
            /*else if (HAMMER_OF_JUSTICE > 0 && !pTarget->HasAura(HAMMER_OF_JUSTICE, EFFECT_0) && CombatCounter < 3 && m_ai->GetManaPercent() >= 3)
               {
            ...
            /*else if (SACRED_SHIELD > 0 && CombatCounter < 6 && pVictim == m_bot && m_ai->GetHealthPercent() < 70 && !m_bot->HasAura(SACRED_SHIELD, EFFECT_0) && m_ai->GetManaPercent() >= 12)
               {
            ...
            /* else if (HAND_OF_SACRIFICE > 0 && pVictim == GetMaster() && !GetMaster()->HasAura(HAND_OF_SACRIFICE, EFFECT_0) && CombatCounter < 10 && m_ai->GetManaPercent() >= 6)
               {
            ...
            /*else if (DIVINE_PROTECTION > 0 && pVictim == m_bot && !m_bot->HasAura(FORBEARANCE, EFFECT_0) && m_ai->GetHealthPercent() < 30 && CombatCounter < 11 && m_ai->GetManaPercent() >= 3)
               {
            ...
            /*else if (DIVINE_PLEA > 0 && !m_bot->HasAura(DIVINE_PLEA, EFFECT_0) && m_ai->GetManaPercent() < 50 && CombatCounter < 13)
               {
            ...
            /*else if (DIVINE_FAVOR > 0 && !m_bot->HasAura(DIVINE_FAVOR, EFFECT_0) && CombatCounter < 14)
               {
    ...
    //if (DIVINE_SHIELD > 0 && m_ai->GetHealthPercent() < 30 && pVictim == m_bot && !m_bot->HasAura(FORBEARANCE, EFFECT_0) && !m_bot->HasAura(DIVINE_SHIELD, EFFECT_0) && m_ai->GetManaPercent() >= 3)
    //    m_ai->CastSpell(DIVINE_SHIELD, *m_bot);

    //if (DIVINE_SACRIFICE > 0 && m_ai->GetHealthPercent() > 50 && pVictim != m_bot && !m_bot->HasAura(DIVINE_SACRIFICE, EFFECT_0))
