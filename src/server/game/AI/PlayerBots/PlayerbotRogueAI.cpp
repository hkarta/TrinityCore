...
/*    if (VANISH > 0 && GetMaster()->isDead()) { //Causes the server to crash :( removed for now.
   ...
   //        m_bot->RemoveAllSpellCooldown();
    ...
    // decide what to do:
    if (pVictim == m_bot && CLOAK_OF_SHADOWS > 0 && pVictim->HasAura(SPELL_AURA_PERIODIC_DAMAGE) && !m_bot->HasAura(CLOAK_OF_SHADOWS, EFFECT_0) && m_ai->CastSpell(CLOAK_OF_SHADOWS))
    {
        if (m_ai->GetManager()->m_confDebugWhisper)
            m_ai->TellMaster("CoS!");
        return true;
    ...
    switch (SpellSequence)
    {
        case RogueStealth:
            ...
            if (PICK_POCKET > 0 && (pTarget->GetCreatureTypeMask() & CREATURE_TYPEMASK_HUMANOID_OR_UNDEAD) != 0 &&
                ...
                out << " > Ambush";
            else if (CHEAP_SHOT > 0 && !pTarget->HasAura(CHEAP_SHOT, EFFECT_0) && m_ai->GetEnergyAmount() >= 60 && m_ai->CastSpell(CHEAP_SHOT, *pTarget))
                out << " > Cheap Shot";
        ...
        case RogueThreat:
            out << "Case Threat";
            if (GOUGE > 0 && m_ai->GetEnergyAmount() >= 45 && !pTarget->HasAura(GOUGE, EFFECT_0) && m_ai->CastSpell(GOUGE, *pTarget))
                out << " > Gouge";
            else if (EVASION > 0 && m_ai->GetHealthPercent() <= 35 && !m_bot->HasAura(EVASION, EFFECT_0) && m_ai->CastSpell(EVASION))
                out << " > Evasion";
            else if (BLIND > 0 && m_ai->GetHealthPercent() <= 30 && !pTarget->HasAura(BLIND, EFFECT_0) && m_ai->GetEnergyAmount() >= 30 && m_ai->CastSpell(BLIND, *pTarget))
                out << " > Blind";
            else if (FEINT > 0 && m_ai->GetHealthPercent() <= 25 && m_ai->GetEnergyAmount() >= 20 && m_ai->CastSpell(FEINT))
                out << " > Feint";
            else if (VANISH > 0 && m_ai->GetHealthPercent() <= 20 && !m_bot->HasAura(FEINT, EFFECT_0) && m_ai->CastSpell(VANISH))
                out << " > Vanish";
            else if (PREPARATION > 0 && m_ai->CastSpell(PREPARATION))
                out << " > Preparation";
            else if (m_bot->getRace() == RACE_NIGHTELF && m_ai->GetHealthPercent() <= 15 && !m_bot->HasAura(SHADOWMELD, EFFECT_0) && m_ai->CastSpell(SHADOWMELD, *m_bot))
                out << " > Shadowmeld";
            else
                out << " NONE!";
            break;
        ...
        default:
            ...
            if (m_bot->GetComboPoints() <= 4)
            {
                if (SHADOW_DANCE > 0 && !m_bot->HasAura(SHADOW_DANCE, EFFECT_0) && m_ai->CastSpell(SHADOW_DANCE, *m_bot))
                    out << " > Shadow Dance";
                else if (CHEAP_SHOT > 0 && m_bot->HasAura(SHADOW_DANCE, EFFECT_0) && !pTarget->HasAura(CHEAP_SHOT, EFFECT_0) && m_ai->GetEnergyAmount() >= 60 && m_ai->CastSpell(CHEAP_SHOT, *pTarget))
                    out << " > Cheap Shot";
                else if (AMBUSH > 0 && m_bot->HasAura(SHADOW_DANCE, EFFECT_0) && m_ai->GetEnergyAmount() >= 60 && m_ai->CastSpell(AMBUSH, *pTarget))
                    out << " > Ambush";
                else if (GARROTE > 0 && m_bot->HasAura(SHADOW_DANCE, EFFECT_0) && m_ai->GetEnergyAmount() >= 50 && m_ai->CastSpell(GARROTE, *pTarget))
                    out << " > Garrote";
                    ...
                    out << " > Arcane Torrent";
                else if ((m_bot->getRace() == RACE_HUMAN && m_bot->HasUnitState(UNIT_STATE_STUNNED)) || m_bot->HasAuraType(SPELL_AURA_MOD_FEAR) || m_bot->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) || (m_bot->HasAuraType(SPELL_AURA_MOD_CHARM) && m_ai->CastSpell(EVERY_MAN_FOR_HIMSELF, *m_bot)))
                    out << " > Every Man for Himself";
                else if ((m_bot->getRace() == RACE_UNDEAD_PLAYER && m_bot->HasAuraType(SPELL_AURA_MOD_FEAR)) || (m_bot->HasAuraType(SPELL_AURA_MOD_CHARM) && m_ai->CastSpell(WILL_OF_THE_FORSAKEN, *m_bot)))
                    ...
                    out << " > Stoneform";
                else if ((m_bot->getRace() == RACE_GNOME && m_bot->HasUnitState(UNIT_STATE_STUNNED)) || (m_bot->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) && m_ai->CastSpell(ESCAPE_ARTIST, *m_bot)))
                    out << " > Escape Artist";
            ...
            else
            {
                ...
                else if (EXPOSE_ARMOR > 0 && pTarget->getClass() == CLASS_WARRIOR && !pTarget->HasAura(EXPOSE_ARMOR, EFFECT_0) && m_ai->GetEnergyAmount() >= 25 && m_ai->CastSpell(EXPOSE_ARMOR, *pTarget))
                    out << " > Warrior Expose Armor";
                else if (EXPOSE_ARMOR > 0 && pTarget->getClass() == CLASS_PALADIN && !pTarget->HasAura(EXPOSE_ARMOR, EFFECT_0) && m_ai->GetEnergyAmount() >= 25 && m_ai->CastSpell(EXPOSE_ARMOR, *pTarget))
                    out << " > Paladin Expose Armor";
                else if (EXPOSE_ARMOR > 0 && pTarget->getClass() == CLASS_DEATH_KNIGHT && !pTarget->HasAura(EXPOSE_ARMOR, EFFECT_0) && m_ai->GetEnergyAmount() >= 25 && m_ai->CastSpell(EXPOSE_ARMOR, *pTarget))
                    out << " > DK Expose Armor";
