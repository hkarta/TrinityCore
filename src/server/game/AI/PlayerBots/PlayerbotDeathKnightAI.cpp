...
bool PlayerbotDeathKnightAI::DoNextCombatManeuver(Unit *pTarget)
{
    ...
    float dist = m_bot->GetDistance(pTarget);//m_bot->GetCombatDistance(pTarget);
    ...
    switch (SpellSequence)
    {
        case SPELL_DK_UNHOLY:
            if (UNHOLY_PRESENCE > 0)
                (!m_bot->HasAura(UNHOLY_PRESENCE, EFFECT_0) && !m_bot->HasAura(BLOOD_PRESENCE, EFFECT_0) && !m_bot->HasAura(FROST_PRESENCE, EFFECT_0) && m_ai->CastSpell (UNHOLY_PRESENCE, *m_bot));

            // check for BONE_SHIELD in combat
            if (BONE_SHIELD > 0)
                (!m_bot->HasAura(BONE_SHIELD, EFFECT_0) && !m_bot->HasAura(ARMY_OF_THE_DEAD, EFFECT_0) && m_ai->CastSpell (BONE_SHIELD, *m_bot));
            ...
            if (ARMY_OF_THE_DEAD > 0 && m_ai->GetAttackerCount() >= 5 && LastSpellUnholyDK < 1)
            {
                ...
                if (ARMY_OF_THE_DEAD > 0 && m_bot->HasAura(ARMY_OF_THE_DEAD, EFFECT_0))
                    m_ai->SetIgnoreUpdateTime(7);
                SpellSequence = SPELL_DK_FROST;
                LastSpellUnholyDK = LastSpellUnholyDK + 1;
                break;
            }
            else if (PLAGUE_STRIKE > 0 && !pTarget->HasAura(PLAGUE_STRIKE, EFFECT_0) && LastSpellUnholyDK < 2)
            {
            ...
            else if (DEATH_GRIP > 0 && !pTarget->HasAura(DEATH_GRIP, EFFECT_0) && LastSpellUnholyDK < 3)
            {
            ...
            else if (DEATH_STRIKE > 0 && !pTarget->HasAura(DEATH_STRIKE, EFFECT_0) && LastSpellUnholyDK < 5)
            {
            ...
            else if (UNHOLY_BLIGHT > 0 && !pTarget->HasAura(UNHOLY_BLIGHT, EFFECT_0) && LastSpellUnholyDK < 6)
            {
            ...
            else if (DEATH_AND_DECAY > 0 && m_ai->GetAttackerCount() >= 3 && dist <= ATTACK_DISTANCE && !pTarget->HasAura(DEATH_AND_DECAY, EFFECT_0) && LastSpellUnholyDK < 8)
            {
            ...
            else if (SUMMON_GARGOYLE > 0 && !m_bot->HasAura(ARMY_OF_THE_DEAD, EFFECT_0) && !pTarget->HasAura(SUMMON_GARGOYLE, EFFECT_0) && LastSpellUnholyDK < 9 && m_ai->GetRunicPower() >= 60)
            {
            ...
            else if (ANTI_MAGIC_SHELL > 0 && pTarget->IsNonMeleeSpellCasted(true) && !m_bot->HasAura(ANTI_MAGIC_SHELL, EFFECT_0) && LastSpellUnholyDK < 11 && m_ai->GetRunicPower() >= 20)
            {
            ...
            else if (ANTI_MAGIC_ZONE > 0 && pTarget->IsNonMeleeSpellCasted(true) && !m_bot->HasAura(ANTI_MAGIC_SHELL, EFFECT_0) && LastSpellUnholyDK < 12)
            {
                ...
                break;
                     ...
                     && (RAISE_DEAD > 0 && !m_bot->HasAura(ARMY_OF_THE_DEAD, EFFECT_0) && LastSpellUnholyDK < 13))
            {
                ...
                break;
                     ...
                     && (GHOUL_FRENZY > 0 && pVictim == pet && !pet->HasAura(GHOUL_FRENZY, EFFECT_0) && LastSpellUnholyDK < 14))
            {
        ...
        case SPELL_DK_FROST:
            if (FROST_PRESENCE > 0)
                (!m_bot->HasAura(FROST_PRESENCE, EFFECT_0) && !m_bot->HasAura(BLOOD_PRESENCE, EFFECT_0) && !m_bot->HasAura(UNHOLY_PRESENCE, EFFECT_0) && m_ai->CastSpell (FROST_PRESENCE, *m_bot));

            if (DEATHCHILL > 0)
                (!m_bot->HasAura(DEATHCHILL, EFFECT_0) && !m_bot->HasAura(KILLING_MACHINE, EFFECT_0) && m_ai->CastSpell (DEATHCHILL, *m_bot));
            else if (KILLING_MACHINE > 0)
                (!m_bot->HasAura(KILLING_MACHINE, EFFECT_0) && !m_bot->HasAura(DEATHCHILL, EFFECT_0) && m_ai->CastSpell (KILLING_MACHINE, *m_bot));

            if (ICY_TOUCH > 0 && !pTarget->HasAura(ICY_TOUCH, EFFECT_0) && LastSpellFrostDK < 1)
            {
            ...
            else if (CHAINS_OF_ICE > 0 && !pTarget->HasAura(CHAINS_OF_ICE, EFFECT_0) && LastSpellFrostDK < 5)
            {
            ...
            else if (ICY_CLUTCH > 0 && !pTarget->HasAura(ICY_CLUTCH, EFFECT_0) && LastSpellFrostDK < 7)
            {
            ...
            else if (ICEBOUND_FORTITUDE > 0 && m_ai->GetHealthPercent() < 50 && pVictim == m_bot && !m_bot->HasAura(ICEBOUND_FORTITUDE, EFFECT_0) && LastSpellFrostDK < 8 && m_ai->GetRunicPower() >= 20)
            {
            ...
            else if (UNBREAKABLE_ARMOR > 0 && !m_bot->HasAura(UNBREAKABLE_ARMOR, EFFECT_0) && m_ai->GetHealthPercent() < 70 && pVictim == m_bot && LastSpellFrostDK < 12)
            {
        ...
        case SPELL_DK_BLOOD:
            if (BLOOD_PRESENCE > 0)
                (!m_bot->HasAura(BLOOD_PRESENCE, EFFECT_0) && !m_bot->HasAura(UNHOLY_PRESENCE, EFFECT_0) && !m_bot->HasAura(FROST_PRESENCE, EFFECT_0) && m_ai->CastSpell (BLOOD_PRESENCE, *m_bot));

            if (MARK_OF_BLOOD > 0 && !pTarget->HasAura(MARK_OF_BLOOD, EFFECT_0) && LastSpellBloodDK < 1)
            {
            ...
            else if (STRANGULATE > 0 && !pTarget->HasAura(STRANGULATE, EFFECT_0) && LastSpellBloodDK < 4)
            {
            ...
            else if (VAMPIRIC_BLOOD > 0 && m_ai->GetHealthPercent() < 70 && !m_bot->HasAura(VAMPIRIC_BLOOD, EFFECT_0) && LastSpellBloodDK < 7)
            {
            ...
            else if (RUNE_TAP > 0 && m_ai->GetHealthPercent() < 70 && !m_bot->HasAura(VAMPIRIC_BLOOD, EFFECT_0) && LastSpellBloodDK < 8)
            {
            ...
            else if (HYSTERIA > 0 && m_ai->GetHealthPercent() > 25 && !m_bot->HasAura(HYSTERIA, EFFECT_0) && LastSpellBloodDK < 9)
            {
            ...
            else if (DANCING_WEAPON > 0 && !m_bot->HasAura(DANCING_WEAPON, EFFECT_0) && m_ai->GetRunicPower() >= 60 && LastSpellBloodDK < 10)
            {
            ...
            else if (DARK_COMMAND > 0 && m_ai->GetHealthPercent() > 50 && pVictim != m_bot && !pTarget->HasAura(DARK_COMMAND, EFFECT_0) && dist <= ATTACK_DISTANCE && LastSpellBloodDK < 11)
            {
